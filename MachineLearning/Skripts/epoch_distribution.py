...
def get_epoch_distribution_for_run(hyperparameters: dict, model_key: str, run_name: str, save_result=True):
    ...
    relevant_splits = [
        ...
        if os.path.basename(path) in ("train_split.csv", "test_split.csv")
    ]
    ...
    label_mapping = metadata.get("epoch_type", {})

    # Converting keys to integers -> ensuring correct datatype
    label_to_name = {int(k): v for k, v in label_mapping.items()}
    ...
    patient_label_counts = defaultdict(lambda: defaultdict(int))  # Dict -> {patient: {label: label count}}
    for _, row in combined_df.iterrows():
        patient_id = row["ResultID"]
        label = int(row["label"])
        label_name = label_to_name.get(label, f"Label_{label}")  # Label according to label dict
        patient_label_counts[patient_id][label_name] += 1
    ...
    if save_result:
        saver = SaveResult()
        saver.save_metadata_analysis(result_df, model_key, hyperparameters, "dataframe", "splits","epoch_distribution", run_name)
