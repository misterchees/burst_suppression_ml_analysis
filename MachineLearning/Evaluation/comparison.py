...
class Comparison:
    ...
    def compare_filtered_and_unfiltered_eeg(self, result_id: int, channel=1, log_scale=True, y_scale="raw"):
        ...
        raw_ax = axes[0]
        filt_ax = axes[1]
        Plots.plot_psd((fig, raw_ax), raw_freq, raw_power, f"raw_EEG of Patient{result_id}", log_scale)
        Plots.plot_psd((fig, filt_ax), filt_freq, filt_power, f"filtered_EEG of Patient{result_id}", log_scale)
